...
def parseBinary(b):
    assert isinstance(b, BinaryOperation)
    L = b.children[1]
    R = b.children[2]
    # print(L)
    if isinstance(L, BinaryOperation):
        L = parseBinary(L)
    if isinstance(R, BinaryOperation):
        R = parseBinary(R)
    if isinstance(L, MemberReference):
        L = L.member
    elif not isinstance(L, str):
        L = L.value
    if isinstance(R, MemberReference):
        R = R.member
    elif not isinstance(R, str):
        R = R.value
    return "(%s %s %s)" % (str(L), b.children[0], str(R))


m = {
    'obj': "XposedHelpers.setObjectField(toObj,%s,XposedHelpers.getObjectField(fromObj,%s));",
    'int': "XposedHelpers.setIntField(toObj,%s,XposedHelpers.getIntField(fromObj,%s));",
    'boolean': "XposedHelpers.setBooleanField(toObj,%s,XposedHelpers.getBooleanField(fromObj,%s));",
    'float': "XposedHelpers.setFloatField(toObj,%s,XposedHelpers.getFloatField(fromObj,%s));",
    'double': "XposedHelpers.setDoubleField(toObj,%s,XposedHelpers.getDoubleField(fromObj,%s));",
    'long': "XposedHelpers.setLongField(toObj,%s,XposedHelpers.getLongField(fromObj,%s));",
    'byte': "XposedHelpers.setByteField(toObj,%s,XposedHelpers.getByteField(fromObj,%s));",
    'short': "XposedHelpers.setShortField(toObj,%s,XposedHelpers.getShortField(fromObj,%s));",
    'char': "XposedHelpers.setCharField(toObj,%s,XposedHelpers.getCharField(fromObj,%s));",
}
                ...
                for clsDeclarationItem in lsItem.children[4]:
                    if isinstance(clsDeclarationItem, FieldDeclaration):
                        ...
                        if not 'static' in l[1]:
                            # print(l)
                            initializer = l[4][0].initializer
                            fieldName = str(l[4][0].name)
                            typeName = 'obj'
                            if isinstance(l[3],BasicType):
                                typeName = l[3].name
                            print(m[typeName]%('"' + fieldName + '"','"' + fieldName + '"'))
